# Legilo

A split-view Markdown reader/editor for Windows, built with Electron. Editor on
the left (CodeMirror 6 with Markdown syntax highlighting), live GitHub-flavored
preview on the right (markdown-it + highlight.js), with synchronized scrolling
and a draggable divider.

*Legilo* is Esperanto for "reading device".

## Features

- **Split view** with a draggable divider, plus editor-only and preview-only
  modes (toolbar button or `Ctrl+1` / `Ctrl+2` / `Ctrl+3`)
- **Tabs** with session restore (`Ctrl+N` / `Ctrl+W` / `Ctrl+Tab`)
- **Presenter mode** (`F5`): Marp-style slides split on `---`
- **Page view** (`Ctrl+Shift+P`): Word-like A4 sheets in the preview showing
  how the document fills printed pages
- **Insert menu** (menu bar + toolbar): headings, bold/italic, links, images
  via a file picker, code blocks, tables, task lists, quotes, footnotes
- **Print** (`Ctrl+P`), **print preview** (opens a PDF), and **Export to
  PDF/HTML**; links open in the system browser
- **Synchronized scrolling** between editor and preview
- **GitHub-flavored markdown**: tables, footnotes, task lists, fenced code
  blocks with syntax highlighting
- **Native menu & shortcuts**: New (`Ctrl+N`), Open (`Ctrl+O`), Save
  (`Ctrl+S`), Save As (`Ctrl+Shift+S`), Export to HTML (`Ctrl+E`)
- **Unsaved-changes guard** before closing or replacing the current document,
  with an asterisk indicator in the window title
- **Light & dark themes** (`Ctrl+Shift+D`), remembered between sessions along
  with window size/position and view mode (via `electron-store`)

## Development

```bash
npm install
npm start          # bundles the renderer with esbuild, then launches Electron
```

On Linux dev machines where Chromium's SUID sandbox isn't set up (error:
`chrome-sandbox is owned by root and has mode 4755`), either run the dev-only
unsandboxed variant:

```bash
npm run start:linux
```

or fix the sandbox helper once per `node_modules` install (survives until the
Electron package is reinstalled/updated):

```bash
sudo chown root:root node_modules/electron/dist/chrome-sandbox
sudo chmod 4755 node_modules/electron/dist/chrome-sandbox
```

This is a dev-machine quirk only; the packaged Windows app is unaffected.

For faster iteration you can keep the renderer bundle rebuilding in a second
terminal and just reload the Electron window (`Ctrl+R` via View → Toggle Dev
Tools console, or restart):

```bash
npm run watch:renderer
```

### Project layout

```
main.js            # Electron main process: window, native menu, dialogs, fs, prefs
preload.js         # contextBridge API (contextIsolation on, sandbox on)
src/renderer.js    # renderer source (bundled by esbuild)
renderer/
  index.html       # app shell
  styles.css       # app chrome + GFM preview styling (light/dark)
  bundle.js        # generated by esbuild — do not edit
```

## Building installers

`npm run dist` builds the installers for the OS you run it on:

| OS | Artifacts |
| --- | --- |
| Windows | NSIS installer (`Legilo Setup <version>.exe`) + AppX (Microsoft Store) |
| macOS | `.dmg` + `.zip` |
| Linux | `.AppImage` + `.deb` |

Notes:

- Each OS builds its own targets — build Windows packages on Windows, macOS
  on a Mac, etc. The GitHub Actions workflow (below) does all three.
- The NSIS installer lets the user pick the install directory, creates a
  desktop shortcut, and associates `.md` files with Legilo.
- macOS/Windows builds are unsigned; macOS users must right-click → Open the
  first time. Code signing can be added later via `CSC_LINK`/notarization.

### Releases via GitHub Actions

Pushing a tag like `v0.2.0` triggers `.github/workflows/release.yml`, which
builds on Windows, macOS and Linux runners and attaches all installers to a
draft GitHub release:

```bash
npm version 0.2.0        # bumps package.json + creates the tag
git push --follow-tags
```

Then publish the draft release on GitHub. A manual run (workflow_dispatch)
only builds and uploads workflow artifacts without creating a release.

### Microsoft Store (AppX)

`npm run dist:store` (on Windows, or take the `.appx` from CI) builds the
Store package. Before the first upload, reserve the app name in
[Partner Center](https://partner.microsoft.com/dashboard) and copy the three
identity values from *Product management → Product identity* into the `appx`
section of `package.json`:

- `identityName` ← *Package/Identity/Name*
- `publisher` ← *Package/Identity/Publisher* (the `CN=…GUID` string)
- `publisherDisplayName` ← *Package/Properties/PublisherDisplayName*

The Store signs the package itself, so no local certificate is needed for
Store submissions.
